...
let is_alpha in_char = (in_char >= 'a' && in_char <= 'z') ||
                       (in_char >= 'A' && in_char <= 'Z')
...
let get_next_char str pos =
  if is_at_end str pos then None
    ...
    let next_char = String.get str pos.current in
...
let try_peek str pos =
  ...
  | Some (next_char, _) -> Some (next_char)
  | None -> None
...
let try_look_ahead str pos target =
  ...
  match result with
  | Some (next_char, next_pos) when next_char = target -> true, next_pos
  | _ -> false, pos
...
let rec scan_string str start_pos =
  ...
  | None -> (start_pos, Error (
    {line = start_pos.line; where="" ;message = "Unterminated string."}))
  | Some next_char ->
  ...
  | '\n' ->
      let next_pos = {start_pos with current = start_pos.current + 1;
                                     line = start_pos.line + 1} in
...
let rec scan_comment str start_pos =
  match try_peek str start_pos with
  | None -> create_token_pos_pair str start_pos Comment
  | Some next_char ->
  match next_char with
  | '\n' ->
      let next_pos = {start_pos with current = start_pos.current + 1;
...
let scan_slash str start_pos =
  let got_match, final_pos = try_look_ahead str start_pos '/' in
  if got_match then
    scan_comment str final_pos
  else
    create_token_pos_pair str final_pos Slash

let create_ident_token str pos =
  let length = pos.current - pos.start in
  let lexeme = String.sub str pos.start length in
  match Hashtbl.find_opt keywords lexeme with
  | None -> create_token_pos_pair str pos (Identifier (lexeme))
  | Some key_type -> create_token_pos_pair str pos key_type

let rec scan_ident str start_pos =
  match try_peek str start_pos with
  | None -> create_ident_token str start_pos
  | Some next_char ->
  match next_char with
  | _ when is_alpha_num next_char ->
      let next_pos = {start_pos with current = start_pos.current + 1} in
      scan_ident str next_pos
  | _ -> create_ident_token str start_pos

let rec scan_token str pos =
  let result = get_next_char str pos in
  match result with
  | None -> pos, Ok({ token_type = EOF; lexeme = ""; line = pos.line})
  | Some(next_char, next_pos) ->
  match next_char with
  ...
  | '\n' ->
      let new_pos = {next_pos with start = next_pos.current; 
                                   line = next_pos.line + 1} in
      scan_token str new_pos
  (* string *)
  | '"' -> scan_string str next_pos
  (* numbers *)
  | _ when is_digit next_char -> scan_number str next_pos
  (* identifier/keywords *)
  | _ when is_alpha next_char ->
      scan_ident str next_pos
  | _ -> (next_pos, Error (
    {line = next_pos.line; where="" ;message = String.concat "" 
...
let rec scan_tokens_aux str pos acc error_acc =
  ...
  match new_tok with
  | Ok ({token_type = EOF; _} as token) -> ((token :: acc), error_acc)
  | Ok token -> scan_tokens_aux str next_pos (token :: acc) error_acc
  | Error err -> scan_tokens_aux str next_pos acc (err :: error_acc)

let scan_tokens str =
  let tokens, errors = scan_tokens_aux str {start=0; current=0; line=1;} 
    [] [] in
  match errors with
  | [] -> Ok(List.rev tokens)
